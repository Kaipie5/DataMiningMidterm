...
numAthletes = 10
labels = data.head(n=numAthletes)['Athlete']
athletes = data.head(n=numAthletes)['Gold']
explode =[]
athleteTotals = []
for i in range(0, numAthletes):
    athleteTotals.append(int(athletes[i]))
    explode.append(0)
    
athleteLabels = {}
for i in range(0, numAthletes):
    athleteLabels[i] = (labels[i])

plt.pie(athleteTotals, explode, athleteLabels.values())
